...
public class Game
{
    ...
    public Game()
    {
       ...
       world.add(ball);
        ...
        box.addComponent(new Body(box, 100, 0.1));
        box.addComponent(new BoxCollider(box, new Dimension(2, 2)));
       // box.rotate(90);
        world.add(box);

        box = new GameObject(new Vector2D(3, 1));
       // box.addComponent(new Rectangle(box, new Dimension(1, 1)));
        box.addComponent(new Body(box, 10000, 0.1));
        box.addComponent(new BoxCollider(box, new Dimension(2, 4)));
       // ball = new GameObject(new Vector2D(0, 2.5));
        //ball.addComponent(new Circle(ball, 5, Color.PINK));
        //ball.addComponent(new Body(ball, 1000, 0.1));
        box.addComponent(new CurveCollider(box, new Vector2D(0, 2.5), 1, false));
        //box.add(ball);
        //box.rotate(90);
        world.add(box);
        ...
        triangle.addComponent(new Body(triangle, 10, 0.1));
        List<Vector2D> corners = new ArrayList<>();
        corners.add(new Vector2D(-1.5, -1));
        corners.add(new Vector2D(1.5, -1));
        corners.add(new Vector2D(0, 1));
        triangle.addComponent(new PolygonCollider(triangle, corners));
        triangle.rotate(5);
        world.add(triangle);

        GameObject hexagon = new GameObject(new Vector2D(-5, 5));
        hexagon.addComponent(new Body(hexagon, 100, 0.1));
        corners = new ArrayList<>();
        corners.add(new Vector2D(0, 2));
        corners.add(new Vector2D(2, 0));
        corners.add(new Vector2D(1.5, -2));
        corners.add(new Vector2D(-1.5, -2));
        corners.add(new Vector2D(-2, 0));
        hexagon.addComponent(new PolygonCollider(hexagon, corners));
        hexagon.rotate(10);
        world.add(hexagon);
    }
